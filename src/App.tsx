...
const DEFAULT_SECTION_VISIBILITY = SECTION_DEFINITIONS.reduce<Record<string, boolean>>((acc, section) => {
  acc[section.id] = true;
  return acc;
}, {});
...
export default function App() {
  ...
  const handleDrop = (event: React.DragEvent<HTMLDivElement>, targetId: string) => {
    event.preventDefault();
    const draggedId = event.dataTransfer.getData('text/plain') || draggedSection;
    ...
    setSectionOrder((previous) => {
      ...
      const insertionIndex = Math.max(0, targetIndex + (shouldInsertAfter ? 1 : 0));
    ...
    <div className="min-h-screen bg-gray-50 py-12 px-4 sm:px-6 lg:px-8 font-sans">
      <div className="max-w-4xl mx-auto">
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          <Section title="Placeholder sections" description="Add temporary cards with reference content.">
            <div className="flex items-center justify-between mb-3">
              <div>
                <p className="text-sm font-semibold text-gray-900">Placeholder sections</p>
                        ...
                        {PLACEHOLDER_PDF_GROUPS.map((group) => (
                          <div key={group.title} className="space-y-2">
                            ...
                            <input
                              ...
                              onChange={(event) => updatePlaceholderSection(index, group.description, event.target.value)}
                              className="w-full rounded-md border border-gray-300 px-3 py-2 text-sm focus:border-indigo-500 focus:ring-2 focus:ring-indigo-200"
                            />
              ...
              return (
                <div
                  ...
                  className={`rounded-3xl transition ${
                    dragOverSection === sectionId ? 'ring-2 ring-indigo-300 shadow-lg' : ''
                  }`}
                >
                  <Section
                    title={
                    ...
                    description={
                      <div className="mt-3 space-y-1">
                        ...
                        <input
                          ...
                          onChange={(event) => updateSectionLabel(sectionId, event.target.value)}
                          className="w-full rounded-md border border-gray-300 px-3 py-2 text-sm focus:border-indigo-500 focus:ring-2 focus:ring-indigo-200"
                        />
                      </div>
                    }
                  ...
                  >
                    {isVisible ? (
                      ...
                      <p className="text-sm text-gray-500">Toggle this section on to configure its content.</p>
                    )}
                  </Section>
                </div>
              );
